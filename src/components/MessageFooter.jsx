...
function MessageFooter({postdata, onReaction, onDeleteReaction, getInfo}) {

    const [likeCount, setLikeCount] = useState(postdata.like_total);
    const [dislikeCount, setDisikeCount] = useState(postdata.dislike_total);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getInfo(postdata.id, 1, 'ASC', 'user_id', localStorage.getItem('user_id'));
                if (resp.state === true && resp.data.totalCount > 0){
                    if (resp.data.rows[0].value === 1){
                        setIsActiveLike(true);
                    }
                    else {
                        setIsActiveDislike(true);
                    }
                }
            } catch (error) {
    ...
    async function like() {
        if (isActiveLike) {
            const resp = await onDeleteReaction(postdata.id, localStorage.getItem('token'));
            if (resp.state === true){
            ...
            const resp = await onReaction(postdata.id, true, localStorage.getItem('token'));
            if (resp.state === true){
                setIsActiveLike(true);
                setLikeCount(likeCount + 1);
            }
    ...
    async function disLike() {
        if (isActiveDislike) {
            ...
            const resp = await onReaction(postdata.id, false, localStorage.getItem('token'));
            if (resp.state === true){
                setIsActiveDislike(true);
                setDisikeCount(dislikeCount + 1);
            }
    ...
    return (
        <div key={`message-${postdata.id}-footer`} className={'message-footer'}>
            <div className={'creation-date'}>{
                new Date(postdata.date_created).toLocaleTimeString().replace(/:\d\d$/, '') + ' ' +
                new Date(postdata.date_created).toLocaleDateString()
            }</div>
            <div className={'right-side'}>
                <div onClick={like} className={isActiveLike ? 'reaction-button like active' : 'reaction-button like'}>
                    <div>
                        {likeCount}
                    </div>
                ...
                <div onClick={disLike} className={isActiveDislike ? 'reaction-button dislike active' : 'reaction-button dislike'}>
